...
var Resources = function (reporter, definition) {
  ...
  this.reporter.beforeRenderListeners.insert({
    after: 'data',
    before: 'scripts'
  }, definition.name, this, Resources.prototype.handleBeforeRender)
...
Resources.prototype.handleBeforeRender = function (request, response) {
  if (!request.template.resources || !request.template.resources.items || request.template.resources.items.length < 1) {
    request.logger.debug('Resources not defined for this template.')
    return
  }

  var self = this

  return Promise.all(request.template.resources.items.map(function (r) {
    return self.reporter.documentStore.collection(r.entitySet).find({shortid: r.shortid}, request).then(function (res) {
      if (res.length < 1) {
    ...
    if (request.options.language || (request.template.resources && request.template.resources.defaultLanguage)) {
      ...
      request.logger.debug('Found ' + applicableResources.length + ' applicable resources.')
