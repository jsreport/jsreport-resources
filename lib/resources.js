...
class Resources {
  ...
  async handleBeforeRender (request, response) {
    ...
    const resources = await Promise.all(request.template.resources.items.map(async (r) => {
      const res = await this.reporter.documentStore.collection(r.entitySet).find({$or: [{shortid: r.shortid}, {name: r.name}]}, request)
      if (res.length < 1) {
        throw this.reporter.createError(`Data item with shortid ${r.shortid} or name ${r.name} was not found (resource lookup)`, {
          statusCode: 404
        })
      }
